...
此测试验证 ClusterEngine 的核心功能，包括：
1.  相似度聚类能否正确识别“相似文件簇”和“独立文件”，并将它们移动到正确的目录。
2.  空目录清理逻辑的正确性。
"""
...
class TestClusterEngine(unittest.TestCase):
    ...
    def test_run_similarity_clustering_moves_alone_files(self, mock_get_keywords, mock_move_files, mock_cosine_similarity):
        """
        v5.4 验证: 测试 run_similarity_clustering 是否能正确处理相似文件和独立文件。
        """
        # --- Arrange ---
        target_dir = "/target"
        threshold = 0.8
        ...
        mock_move_files.assert_has_calls([call_for_similar, call_for_alone], any_order=True)
